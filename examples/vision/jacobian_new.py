...
def example_jacobian(model_ori, x0, bound_opts, device):
    ...
    for eps in [0, 1./255, 4./255]:
        ...
        lower, upper = model.compute_jacobian_bounds_new(x)
...
def example_local_lipschitz(model_ori, x0, bound_opts, device):
    ...
    mask[1, 0] = 1
    model = BoundedModule(LocalLipschitzWrapper(model_ori), (x0, mask),
    ...
    for eps in [0, 1./255, 4./255]:
        ...
        for i in range(mask.shape[0]):
            mask.zero_()
            mask[i, 0] = 1
            ub = model.compute_jacobian_bounds_new((x, mask), bound_lower=False)[1]
            lip.append(ub)
...
def example_jvp(model_ori, x0, bound_opts, device):
    ...
    for eps in [0, 1./255, 4./255]:
        x = BoundedTensor(x0, PerturbationLpNorm(norm=np.inf, eps=eps))
        lb, ub = model.compute_jacobian_bounds_new((x, vector))
...
def compute_jacobians_new(model_ori, x0, bound_opts=None, device='cpu'):
...
if __name__ == '__main__':
    ...
    compute_jacobians_new(model_ori, x0, device=device)
